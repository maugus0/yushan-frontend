...
const mockNovels = [
  ...
  ...Array.from({ length: 90 }, (_, i) => {
    const id = i + 11;
    const categories = ['Action', 'Adventure', 'Martial Arts', 'Fantasy', 'Sci-Fi', 'Urban', 'Historical', 'Eastern Fantasy', 'Wuxia', 'Xianxia', 'Military', 'Sports', 'Romance', 'Drama', 'Slice of Life', 'School Life', 'Comedy'];
...
const mockUsers = [
  ...
  ...Array.from({ length: 95 }, (_, i) => {
    ...
    if (rand < 0.1) { // 10% Legend (Level 5)
      xp = Math.floor(Math.random() * 5000) + 5000;
      level = 5;
    } else if (rand < 0.25) { // 15% Master Reader (Level 4)
      xp = Math.floor(Math.random() * 3000) + 2000;
      level = 4;
    } else if (rand < 0.5) { // 25% Enthusiast (Level 3)
      xp = Math.floor(Math.random() * 1500) + 500;
      level = 3;
    } else if (rand < 0.75) { // 25% Explorer (Level 2)
      xp = Math.floor(Math.random() * 400) + 100;
      level = 2;
    } else { // 25% Newbie (Level 1)
